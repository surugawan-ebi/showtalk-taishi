...
- Keep Store release approvals bound to the exact repository-local release command shown to the operator. Credentials and release configuration must remain outside this repository.
...
- After restarting a worker for an approval-bridge change, do not report the fix verified from unit tests or reconnect logs alone. Complete the live acceptance gate in `docs/gateway-restart-verification.md`.
